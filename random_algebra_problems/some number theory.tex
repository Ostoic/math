...
\begin{proof}
\case{1} Suppose both $ a $ and $ b $ are quadratic residues modulo $ p $. Then by Lemma 2 above, we can write $ a \equiv g^{2i} $ and $ b \equiv g^{2j} $ for some $ i $ and $ j $, and where $ g $ is a primitive root modulo $ p $. It follows that $ ab \equiv g^{2i} g^{2j} \equiv g^{2(i + j)} $, hence $ ab $ is a quadratic residue by the lemma.
